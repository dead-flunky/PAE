...
class WBUnitScreen:
    ...
    def doAllCommands(self, pUnitX, iIndex):
        if iIndex == 1:
            ...
            for i in xrange(iChange + 1):
                pNewUnit = gc.getPlayer(pUnitX.getOwner()).initUnit(pUnitX.getUnitType(), pUnitX.getX(), pUnitX.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.NO_DIRECTION, pUnitX.getEthnic(), pUnitX.getReligion())
    ...
    def changeOwner(self, iPlayer):
        pNewUnit = gc.getPlayer(iPlayer).initUnit(pUnit.getUnitType(), pUnit.getX(), pUnit.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.NO_DIRECTION, pUnit.getEthnic(), pUnit.getReligion())
    ...
    def changeUnitType(self, pUnit, iUnitType, bUnitType):
        if bUnitType:
            pNewUnit = gc.getPlayer(pUnit.getOwner()).initUnit(iUnitType, pUnit.getX(), pUnit.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.NO_DIRECTION, pUnit.getEthnic(), pUnit.getReligion())
            pNewUnit.convert(pUnit)
