...
# "bonus": index of bonus stored in merchant (only one at a time)
# "cultivationBonuses": indices of bonuses stored in cultivation unit, e.g. "23,21,4"
    ...
    if not bInitialised:
        ...
        for i in range(iRange):
          ...
          if loopPlayer.isAlive():
            ...
            while pLoopCity:
              # Never None, but maybe all whitespace -> strip
              if CvUtil.getScriptData(pLoopCity, ["TradeSpecialBonus"], "-1") != -1:
...
def doCultivateBonusFromCity(pUnit, eBonus):
  ...
  iRand = CvEventInterface.getEventManager().myRandom(len(lPlotList), None)
        ...
        if not eBonus in lBonusList:
          ...
          if iLocalPrice != -1 and iLocalPrice <= iPrice:
            ...
            if doCollectBonus4Cultivation(pUnit,eBonus):
              bBought = True
              #CyInterface().addMessage(gc.getGame().getActivePlayer(), True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_TEST",("eingekauft",iPlayer)), None, 2, None, ColorTypes(10), 0, 0, False, False)
            #else:
              #CyInterface().addMessage(gc.getGame().getActivePlayer(), True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_TEST",("Einkauf von "+str(eBonus)+" fehlgeschlagen",iPlayer)), None, 2, None, ColorTypes(10), 0, 0, False, False)
        #if pLocalCity:
        #  CyInterface().addMessage(gc.getGame().getActivePlayer(), True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_TEST",(pUnit.getName()+" "+pLocalCity.getName()+" "+pLoopCity.getName(),iPlayer)), None, 2, None, ColorTypes(10), 0, 0, False, False)
        # schon am Ziel?
        if pLocalCity and pLoopCity.getID() == pLocalCity.getID():
          # eingekauft?
          if bBought:
            if doCultivateBonusFromCity(pUnit, eBonus):
              #CyInterface().addMessage(gc.getGame().getActivePlayer(), True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_TEST",("verbreitet",iPlayer)), None, 2, None, ColorTypes(10), 0, 0, False, False)
              return True
          #CyInterface().addMessage(gc.getGame().getActivePlayer(), True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_TEST",(pUnit.getName()+" am Ziel ohne Ressi",iPlayer)), None, 2, None, ColorTypes(10), 0, 0, False, False)
...
def doCollectBonus(pUnit):
  if pUnit == None or pUnit.isNone(): return
  iTeam = pUnit.getTeam()
  pPlot = pUnit.plot()
  if pPlot == None or pPlot.isNone(): return
...
def doBuyBonus(pUnit, eBonus, iCityOwner):
    ...
    CvUtil.addScriptData(pUnit, "bonus", eBonus)
...
def doSellBonus(pUnit, pCity):
    eBonus = int(CvUtil.getScriptData(pUnit, ["bonus"], -1))
    if eBonus == -1: return
    ...
    if iOriginCiv != iBuyer:
        ...
        if eBonus in lLuxury + lRarity:
          ...
          if not hasBonusIgnoreFreeBonuses(pCity, eBonus) and pUnit.getDomainType != gc.getInfoTypeForString("DOMAIN_SEA"):
            ...
            if CvEventInterface.getEventManager().myRandom(100, None) < iChance:
              ...
              if pOriginPlot != None and not pOriginPlot.isNone():
                if pOriginPlot.isCity():
                    ...
                    # Sobald von einer Stadt 3 Handelsstrassen (bzw 2 bei einer Küstenstadt) weggehen,
                    ...
                    if not pOriginCity.isHasBuilding(iBuilding):
                      iAnz = 0
                      iMax = 3
                      if pOriginCity.isCoastal(3): iMax = 2
                      for x in range(-2, 2):
                        for y in range(-2, 2):
...
def initCultivationUnit(pUnit, pCity):
    ...
    #~ if gc.getPlayer(pUnit.getOwner()).isHuman():
       #~ if len(lStored) > 0:
         #~ CyInterface().addMessage(pUnit.getOwner(), True, 5, CyTranslator().getText("TXT_KEY_MESSAGE_TRADE_COLLECT_GOODS",(gc.getBonusInfo(eBonus).getDescription(),)), "AS2D_BUILD_GRANARY", 2, None, ColorTypes(13), 0, 0, False, False)
       #~ else:
...
def initAutomatedTradeRoute(pUnit, iX1, iY1, iX2, iY2, eBonus1, eBonus2):
    ...
    CvUtil.addScriptData(pUnit, "automActive", True)
...
##            break
    ...
    bActive = bool(CvUtil.getScriptData(pUnit, ["automActive"], False)) # set to False if automated route is deactivated
    ...
    eStoredBonus = int(CvUtil.getScriptData(pUnit, ["bonus"], -1))
    ...
    if (iUnitX == iX1 and iUnitY == iY1) or (iUnitX == iX2 and iUnitY == iY2):
        if iUnitX == iX1 and iUnitY == iY1:
            pCurrentCity = pCity1
            pNewCity = pCity2
            eBonusBuy = eBonus1
            eBonusSell = eBonus2
        ...
        if eBonusBuy == -1:
            ...
            CvUtil.addScriptData(pUnit, "automActive", False) # deactivate route
...
def doUpdateCitiesWithSpecialBonus():
    ...
    for pCity in lCitiesSpecialBonus:
      if pCity and not pCity.isNone():
        ...
        iTurn -= 1
        if iTurn <= 0:
          eBonus = int(CvUtil.getScriptData(pCity, ["TradeSpecialBonus"],-1))
          CvUtil.removeScriptData(pCity, "TradeSpecialBonus")
          CvUtil.removeScriptData(pCity, "TradeSpecialTurns")
          ...
          CvUtil.addScriptData(pCity, "TradeSpecialTurns", iTurn)
    return
...
def addCityWithSpecialBonus():
    ...
    if len(lCitiesSpecialBonus) < 3:
      ...
      if len(lNewCities):
        # Stadt auswaehlen
        iRand = CvEventInterface.getEventManager().myRandom(len(lNewCities), None)
        ...
        # Runden setzen
        lTurns = [21,26,31]
        iRand = CvEventInterface.getEventManager().myRandom(len(lTurns), None)
        CvUtil.addScriptData(pCity, "TradeSpecialTurns", lTurns[iRand])
        # Bonusgut herausfinden
        lNewBonus = []
        for iBonus in lLuxury + lRarity:
          if not pCity.hasBonus(iBonus): lNewBonus.append(iBonus)
        # Bonus setzen
        iRand = CvEventInterface.getEventManager().myRandom(len(lNewBonus), None)
        CvUtil.addScriptData(pCity, "TradeSpecialBonus", lNewBonus[iRand])
        # Message
        if gc.getTeam(gc.getPlayer(gc.getGame().getActivePlayer()).getTeam()).isHasMet(gc.getPlayer(pCity.getOwner()).getTeam()):
...
def doCheckCitySpecialBonus(pUnit,pCity,eBonus):
    ...
    if eCityBonus != -1 and eCityBonus == eBonus:
        ...
        # Unit as gift:
        if CvEventInterface.getEventManager().myRandom(5, None) == 1:
            lGift = []
            # Auxiliar
        ...
        # Choose unit
        iRand = CvEventInterface.getEventManager().myRandom(len(lGift), None)
        ...
        if iNewUnit == gc.getInfoTypeForString("UNIT_SUPPLY_FOOD"):
            ...
            lBonusList = [lGoods[CvEventInterface.getEventManager().myRandom(len(lGoods), None)]]
            CvUtil.addScriptData(pUnit1, "cultivationBonuses", convertIntListToString(lBonusList))
            lBonusList = [lGoods[CvEventInterface.getEventManager().myRandom(len(lGoods), None)]]
            CvUtil.addScriptData(pUnit2, "cultivationBonuses", convertIntListToString(lBonusList))
