        ...
        # Unit wird verkauft (beim Soeldnerposten) - Sell unit
            ...
            # 695, 1, 0, iOwner, iUnitID
                ...
                if pUnit.isHasPromotion(gc.getInfoTypeForString("PROMOTION_MERCENARY")):
                    iCost = iCost / 2
                iGold = CvUtil.myRandom(iCost-4, "695") + 5
...
# self.doAutomateMerchant(pUnit)
        ...
        # 752: bless units
            ...
            PAE_Unit.doBlessUnits(iData2, iData3, iData4)
...
# Soll nur alle 4 Runden pruefen
        bRevolt = False
        if iGameTurn % 4 == 0 and iPlayer != -1 and iPlayer != gc.getBARBARIAN_PLAYER():
            pPlayer = gc.getPlayer(iPlayer)
...
# ------ Anarchie wenn mehr als die Haelfte der Staedte revoltieren (mehr als 1 Stadt) oder von der Pest heimgesucht werden - 33 %
        if pPlayer.getAnarchyTurns() <= 0:
            ...
            if iRand == 1:
                ...
                if iNumCities > 0:
                    for i in range(iNumCities):
                        if pPlayer.getCity(i).isHasBuilding(iBuilding):
                            iCityPlague += 1
                        if pPlayer.getCity(i).getOccupationTimer() > 0:
                            iCityRevolt += 1
                ...
                if not bDontGoBlackAnymore:
                    ...
                    for iI in range(iRange):
                        ...
                        if not pPlot.isVisible(iTeam, 0):
                            ...
                            if pPlot.isCity():
                                pCity = pPlot.getPlotCity()
                                if bShowCoasts and pCity.isCapital():
                                    bGoBlack = False
                ...
                if gc.getTeam(pPlayer.getTeam()).isHasTech(gc.getInfoTypeForString("TECH_SOELDNERTUM")):
                    ...
                    if iGold > 400:
                        ...
                        if iGold > 800:
                            ...
                            iChance = 10
                        if iChance > CvUtil.myRandom(100, "AI Mercenaries"):
                            bDoIt = True

                        if bDoIt:
                            PAE_Mercenaries.doAIPlanAssignMercenaries(iPlayer)
                            # 20% gleich nochmal
                            if 0 == CvUtil.myRandom(5, "AI Mercenaries2"):
                                PAE_Mercenaries.doAIPlanAssignMercenaries(iPlayer)
                                # 10% ein drittes Mal damits interessant wird ;)
                                if 0 == CvUtil.myRandom(10, "AI Mercenaries3"):
                                    PAE_Mercenaries.doAIPlanAssignMercenaries(iPlayer)
...
# --------- Feature - Seuche auf dem Schlachtfeld ----------------------
        ...
        if gc.getMap().plot(pLoser.getX(), pLoser.getY()).getNumUnits() > 3:
            ...
            iRand = CvUtil.myRandom(50, "FEATURE_SEUCHE")
            if iRand == 1:
                if pPlot1.getFeatureType() == -1 and not pPlot1.isCity() and not pPlot1.isWater():
                    pPlot1.setFeatureType(feat_seuche, 0)
                if pPlot2.getFeatureType() == -1 and not pPlot2.isCity() and not pPlot2.isWater():
                    pPlot2.setFeatureType(feat_seuche, 0)
# --------------
        ...
        if bNavalUnit:
            if pLoser.getUnitType() == gc.getInfoTypeForString("UNIT_TREIBGUT"):
                ...
                if pWinner.getCargo() < pWinner.cargoSpace():
                    ...
                    if iRand < 2:
                        iRand = CvUtil.myRandom(2, "Treibgut einfangen2")
                        if iRand == 0:
...
# ------- Bestimmte Tiere koennen eingefangen werden, wenn Domestizier-Tech erforscht wurde
        if pLoser.getUnitType() in AnimalArray:
            iTech = -1
            if pLoser.getUnitType() == gc.getInfoTypeForString("UNIT_WILD_HORSE"):
...
# ------- Loser Unit Elephant makes 10% collateral damage to friendly units
        if pLoser.getUnitType() == gc.getInfoTypeForString("UNIT_WAR_ELEPHANT"):
            ...
            for iLoserUnits in range(iRange):
                if pLoserPlot.getUnit(iLoserUnits).getDamage() + 20 < 100:
                    pLoserPlot.getUnit(iLoserUnits).changeDamage(20, False)
            if gc.getPlayer(pWinner.getOwner()).isHuman():
                ...
                if pLoser.getCaptureUnitType(pLoser.getCivilizationType()) == -1:
                    ...
                    if team.isHasTech(iJagd):
                        ...
                        if len(CityArray) > 0:
                            ...
                            if pWinner.getUnitType() == gc.getInfoTypeForString("UNIT_HUNTER"):
        ...
        if not bUnitDone:
            ...
            if pLoser.getLeaderUnitType() > -1:
                ...
                if gc.getPlayer(pLoser.getOwner()).isHuman() and gc.getPlayer(pLoser.getOwner()).isTurnActive() or gc.getPlayer(pWinner.getOwner()).isHuman() and gc.getPlayer(pWinner.getOwner()).isTurnActive():
                    ...
                    if gc.getPlayer(pLoser.getOwner()).getCurrentEra() > 2 or gc.getPlayer(pWinner.getOwner()).getCurrentEra() > 2:
                        iVids = 13
                    else:
                        iVids = 11
            ...
            if pCity.isHasBuilding(iBuilding):
                    ...
                    for j in range(3):
                        ...
                        for iUnit in range(iRange):
                            if loopPlot.getUnit(iUnit).isMilitaryHappiness():
                                if pTeam.isAtWar(gc.getPlayer(loopPlot.getUnit(iUnit).getOwner()).getTeam()):
                                    iUnitAnzahl += 1
                ...
                for i in range(iRange):
                    if pPlot.getUnit(i).canFight():
                        iUnitCity += 1
                        # loyal units ?
                        if pPlot.getUnit(i).isHasPromotion(iPromoLoyal):
                            iChanceUnits += 3
                        else:
                            iChanceUnits += 1
                ...
                if bCityRenegade and not pCity.isNone():
                    ...
                    # City renegades
                    PAE_City.doRenegadeCity(pCity, iWinner, pLoser.getID())
...
# --------- if unit will renegade ----------------------
        ...
        if not bUnitDone and not bCityRenegade:
            ...
            if pWinner.getUnitAIType() == UnitAITypes.UNITAI_ANIMAL:
                ...
                bUnitRenegades = False
            elif pWinner.getUnitAIType() == UnitAITypes.UNITAI_EXPLORE:
                bUnitRenegades = False
            elif pLoser.getUnitAIType() == UnitAITypes.UNITAI_EXPLORE:
                bUnitRenegades = False
            elif pLoser.getUnitType() in UnitArray:
                bUnitRenegades = False
            elif pLoser.getUnitType() in AnimalArray:
                bUnitRenegades = False
            elif pWinner.getUnitType() in WildAnimals:
                bUnitRenegades = False
            elif pLoser.getUnitType() in WildAnimals:
                bUnitRenegades = False
        ...
        if bUnitRenegades:
            ...
            if bUnitRenegades:
                ...
                if gc.getPlayer(pWinner.getOwner()).hasTrait(gc.getInfoTypeForString("TRAIT_CHARISMATIC")):
                    iUnitRenegadeChance += 10
                if gc.getPlayer(pLoser.getOwner()).hasTrait(gc.getInfoTypeForString("TRAIT_CHARISMATIC")):
                    iUnitRenegadeChance -= 10
                ...
                if iRand < iUnitRenegadeChance:
                    ...
                    if CvUtil.myRandom(3, "actualRenegade") == 0:
                        ...
                        if pLoser.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_NAVAL"):
                            if pLoser.getUnitType() == gc.getInfoTypeForString("UNIT_PIRAT_KONTERE"):
                                iNewUnitType = gc.getInfoTypeForString("UNIT_KONTERE")
                            elif pLoser.getUnitType() == gc.getInfoTypeForString("UNIT_PIRAT_BIREME"):
                                iNewUnitType = gc.getInfoTypeForString("UNIT_BIREME")
                            elif pLoser.getUnitType() == gc.getInfoTypeForString("UNIT_PIRAT_TRIREME"):
                                iNewUnitType = gc.getInfoTypeForString("UNIT_TRIREME")
                            elif pLoser.getUnitType() == gc.getInfoTypeForString("UNIT_PIRAT_LIBURNE"):
        ...
        if not bUnitDone and (pLoser.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_MOUNTED") or pLoser.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_CHARIOT")):
            ...
            if bDoIt:
                ...
                if iRand == 0 and iNewUnitType != -1:
                    ...
                    if len(rebelPlotArray) > 0:
                        barbPlayer = gc.getPlayer(gc.getBARBARIAN_PLAYER())
                        iPlot = CvUtil.myRandom(len(rebelPlotArray), None)
...
# +++++++++++++++++++++++++++++++++++++
        ...
        if iImprovement > -1 and not bAnimal:
            if iImprovement != gc.getInfoTypeForString("IMPROVEMENT_CITY_RUINS") and iImprovement != gc.getInfoTypeForString("IMPROVEMENT_GOODY_HUT"):
                ...
                # Chance calculation
                if CvUtil.myRandom(100, None) < iChance:
        ...
        if not pWinnerTeam.isHasTech(iTech):
            if gc.getPlayer(pWinner.getOwner()).canResearch(iTech, False):
                ...
                if iCost <= 1:
                    ...
                    iCost = iCost + CvUtil.myRandom(iCost, None)
...
#    if not gc.getGame().isNetworkMultiPlayer() and gc.getPlayer(pCity.getOwner()).isHuman() and isWorldWonderClass(gc.getBuildingInfo(iBuildingType).getBuildingClassType()):
                ...
                for j in range(3):
                    ...
                    if None != loopPlot and not loopPlot.isNone():
                        if loopPlot.getTerrainType() == terrain_swamp:
                            ...
                            bFeatSwamp = True
            if bFeatSwamp and gc.getPlayer(pCity.getOwner()).isHuman():
        ...
        if iBuildingType == iBuilding and CvUtil.myRandom(2, None) == 1:
            ...
            for i in range(5):
                ...
                for j in range(5):
                    ...
                    if loopPlot is not None and not loopPlot.isNone() and not (i == 0 and j == 0) and not (i == 4 and j == 0) and not (i == 0 and j == 4) and not (i == 4 and j == 4):
                        ...
                        if loopPlot.getTerrainType() != terr_swamp and loopPlot.getFeatureType() != feat_flood_plains and not loopPlot.isPeak():
                            if loopPlot.getBonusType(loopPlot.getOwner()) == -1 and loopPlot.getBonusType(-1) == -1 and (loopPlot.getTerrainType() == terr_plain or loopPlot.getTerrainType() == terr_grass) and (loopPlot.getOwner() == pCity.getOwner() or loopPlot.getOwner() == -1):
                                ...
                                if loopPlot.isCity():
                                    ...
                                    if loopPlot.getTerrainType() == terr_plain and loopPlot.isHills() and loopPlot.getImprovementType() == -1:
                                        PlotPrio1.append(loopPlot)
                                    # 2. grass and hills, unworked
                                    if loopPlot.getTerrainType() == terr_grass and loopPlot.isHills() and loopPlot.getImprovementType() == -1:
            ...
            if setWinery and len(correctPlotArray) > 0:
                ...
                if gc.getPlayer(pCity.getOwner()).isHuman():
                    iRand = 1 + CvUtil.myRandom(4, None)
                        ...
                        if loopPlot.getTerrainType() != terr_swamp and loopPlot.getFeatureType() != feat_flood_plains and not loopPlot.isPeak() and loopPlot.getBonusType(loopPlot.getOwner()) == -1 and (loopPlot.getTerrainType() == terr_plain or loopPlot.getTerrainType() == terr_grass) and (loopPlot.getOwner() == pCity.getOwner() or loopPlot.getOwner() == -1):
                            ...
                            if loopPlot.isCity():
                                PlotPrio4.append(loopPlot)
                                    ...
                                    # 1. plain and hills, unworked
                                if loopPlot.getTerrainType() == terr_plain and not loopPlot.isHills() and loopPlot.getImprovementType() == -1:
                                    PlotPrio1.append(loopPlot)
                                # 2. grass and hills, unworked
                                if loopPlot.getTerrainType() == terr_grass and not loopPlot.isHills() and loopPlot.getImprovementType() == -1:
                                    PlotPrio2.append(loopPlot)
                                # 3. irgendeinen passenden ohne Improvement
                                if loopPlot.getImprovementType() == -1:
                                    PlotPrio3.append(loopPlot)
                ...
                for j in range(5):
                    ...
                    if loopPlot is not None and not loopPlot.isNone():
                        ...
                        if loopPlot.getBonusType(loopPlot.getOwner()) == -1 and (loopPlot.getTerrainType() == terr_plain or loopPlot.getTerrainType() == terr_grass) and (loopPlot.getOwner() == pCity.getOwner() or loopPlot.getOwner() == -1):
                            ...
                            # 1. jungle, unworked
                            if loopPlot.getFeatureType() == feat_jungle and loopPlot.getImprovementType() == -1:
                                PlotPrio1.append(loopPlot)
                            # 2. savanna, unworked
                            elif loopPlot.getFeatureType() == feat_savanna and loopPlot.getImprovementType() == -1:
                                PlotPrio2.append(loopPlot)
                            # 3. nach Improvements selektieren
                            elif loopPlot.getImprovementType() == iImpType1:
                                PlotPrio3.append(loopPlot)
                            # 4. grass, unworked
                            elif loopPlot.getTerrainType() == terr_grass and loopPlot.getImprovementType() == -1:
                                PlotPrio4.append(loopPlot)
                            # 5. plains, unworked
                            elif loopPlot.getTerrainType() == terr_plain and loopPlot.getImprovementType() == -1:
                                PlotPrio5.append(loopPlot)
        ...
        if iBuildingType == iBuilding:
            ...
            while iCities > 0 and a > 0:
                iRand = CvUtil.myRandom(iCities, None)
    ...
    def onSelectionGroupPushMission(self, argsList):
        ...
        if eMission == MissionTypes.MISSION_TRADE:
            ...
            if pUnit.canMove():
                ...
                if CvUtil.myRandom(100, None) < 2:
                    scriptCityId = CvUtil.getScriptData(pUnit, ["c", "t"])  # CityID
                    if scriptCityId != "":
                        pSource = gc.getPlayer(eOwner).getCity(int(scriptCityId)).plot()
                    else:
                        pSource = gc.getPlayer(eOwner).getCapitalCity().plot()
        ...
        if pUnit.getDomainType() == DomainTypes.DOMAIN_SEA:
            ...
            if pPlot.getFeatureType() > -1:
                ...
                if iPlotWind in lSeewind:
                    ...
                    if iUnitDirection == 0:
                        if iPlotWind == feat_wind_n:
                            pUnit.changeMoves(iGegenWind)
                        elif iPlotWind == feat_wind_ne or iPlotWind == feat_wind_nw:
                            pUnit.changeMoves(iGegenSchraegWind)
                        elif iPlotWind == feat_wind_e or iPlotWind == feat_wind_w:
                            pUnit.changeMoves(iSeitenWind)
                        elif iPlotWind == feat_wind_se or iPlotWind == feat_wind_sw:
                            pUnit.changeMoves(iInSchraegWind)
                        elif iPlotWind == feat_wind_s:
                            pUnit.changeMoves(iInWind)
                    ...
                    # Bewegung nach Osten
                        ...
                        if iPlotWind == feat_wind_e:
                            pUnit.changeMoves(iGegenWind)
                        elif iPlotWind == feat_wind_ne or iPlotWind == feat_wind_se:
                            pUnit.changeMoves(iGegenSchraegWind)
                        elif iPlotWind == feat_wind_n or iPlotWind == feat_wind_s:
                            pUnit.changeMoves(iSeitenWind)
                        elif iPlotWind == feat_wind_nw or iPlotWind == feat_wind_sw:
                            pUnit.changeMoves(iInSchraegWind)
                        elif iPlotWind == feat_wind_w:
                            pUnit.changeMoves(iInWind)
                    ...
                    # Bewegung nach Nordosten
                        ...
                        if iPlotWind == feat_wind_ne:
                            pUnit.changeMoves(iGegenWind)
                        elif iPlotWind == feat_wind_n or iPlotWind == feat_wind_e:
                            pUnit.changeMoves(iGegenSchraegWind)
                        elif iPlotWind == feat_wind_nw or iPlotWind == feat_wind_se:
                            pUnit.changeMoves(iSeitenWind)
                        elif iPlotWind == feat_wind_w or iPlotWind == feat_wind_s:
                            pUnit.changeMoves(iInSchraegWind)
                        elif iPlotWind == feat_wind_sw:
                            pUnit.changeMoves(iInWind)
                    ...
                    # Bewegung nach Nordwesten
                        ...
                        if iPlotWind == feat_wind_nw:
                            pUnit.changeMoves(iGegenWind)
                        elif iPlotWind == feat_wind_n or iPlotWind == feat_wind_w:
                            pUnit.changeMoves(iGegenSchraegWind)
                        elif iPlotWind == feat_wind_sw or iPlotWind == feat_wind_ne:
                            pUnit.changeMoves(iSeitenWind)
                        elif iPlotWind == feat_wind_e or iPlotWind == feat_wind_s:
                            pUnit.changeMoves(iInSchraegWind)
                        elif iPlotWind == feat_wind_se:
                            pUnit.changeMoves(iInWind)
            ...
            # Schiffe auf Hoher See erleiden Sturmschaden
                ...
                if iDamage <= 90 - iSchaden:
                    ...
                    if 1 == CvUtil.myRandom(iChance, None):
...
# --------------------------------------------------------------------- #
        ...
        if pUnit.getUnitType() == gc.getInfoTypeForString("UNIT_TRADE_MERCHANT") and not pUnit.isBarbarian():
            if pPlot.getNumUnits() == 1 and pUnit.getOwner() != pPlot.getOwner() and not pPlot.isCity():
                ...
                iRand = CvUtil.myRandom(iCalcChance, None)
                if iRand == 1:
                    iPlayer = pUnit.getOwner()
                    if gc.getPlayer(iPlayer).isHuman():
                        iRand = CvUtil.myRandom(5, None)
                        if iRand == 1:
    ...
    def onUnitBuilt(self, argsList):
        ...
        if not pPlayer.isHuman():
            ...
            if unit.getUnitType() in UnitsA or unit.getUnitType() in UnitsB:
                ...
                for i in range(iRange):
                    if not pPlot.getUnit(i).isNone():
                        if pPlot.getUnit(i).getUnitType() == unit.getUnitType() and pPlot.getUnit(i).getOwner() == city.getOwner():
                            iNum += 1
                # UnitAIType 10 = UNITAI_CITY_DEFENSE
                if unit.getUnitType() in UnitsA and iNum < 3 or unit.getUnitType() in UnitsB and iNum < 2:
            ...
            if iHandicap >= 5:
                if unit.isMilitaryHappiness() or unit.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_NAVAL"):
                    ...
                    iRange = len(lPromos)
                    iRand = CvUtil.myRandom(iRange, None)
                    if iRand > 0:
                        for i in range(iRange):
                            if i > 0 and i <= iRand:
        ...
        if unit.getUnitType() == gc.getInfoTypeForString("UNIT_EMIGRANT"):
            ...
            if iCultureMax > 1000:
                iCultureMax = 1000
            iCulture = iPop * 100
            if iCulture > iCultureMax:
                iCulture = iCultureMax
...
#    CvUtil.pyPrint('Unit Promotion Event: %s - %s' %(player.getCivilizationName(), pUnit.getName(),))
    ...
    def onUnitSelected(self, argsList):
        ...
        PAE_Unit.onUnitSelected(pUnit)

        # if pUnit.isBarbarian():
        # PAE_Barbaren.onUnitSelected(pUnit)
    ...
    def onUnitPillage(self, argsList):
        ...
        if iImprovement > -1:
            ...
            if not pUnit.isHasPromotion(gc.getInfoTypeForString("PROMOTION_PILLAGE5")):
                if 2 > CvUtil.myRandom(10, "pillage_promo"):
                    if pUnit.isHasPromotion(gc.getInfoTypeForString("PROMOTION_PILLAGE4")):
                        iNewPromo = gc.getInfoTypeForString("PROMOTION_PILLAGE5")
                    elif pUnit.isHasPromotion(gc.getInfoTypeForString("PROMOTION_PILLAGE3")):
                        iNewPromo = gc.getInfoTypeForString("PROMOTION_PILLAGE4")
                    elif pUnit.isHasPromotion(gc.getInfoTypeForString("PROMOTION_PILLAGE2")):
                        iNewPromo = gc.getInfoTypeForString("PROMOTION_PILLAGE3")
                    elif pUnit.isHasPromotion(gc.getInfoTypeForString("PROMOTION_PILLAGE1")):
                        iNewPromo = gc.getInfoTypeForString("PROMOTION_PILLAGE2")
                    else:
                        iNewPromo = gc.getInfoTypeForString("PROMOTION_PILLAGE1")
...
#    CvUtil.pyPrint('%s received a goody' %(gc.getPlayer(iPlayer).getCivilizationDescription(0)),)
    ...
    def onGreatPersonBorn(self, argsList):
        ...
        if pUnit.getUnitType() == gc.getInfoTypeForString("UNIT_GREAT_GENERAL"):
            ...
            while i < listlength:
                i += 1
                iRand = CvUtil.myRandom(listlength)
            ...
            if GG_Name == "":
                iRand = CvUtil.myRandom(len(listNamesStandard))
...
#    CvUtil.pyPrint('A %s was born for %s in %s' %(pUnit.getName(), player.getCivilizationName(), pCity.getName()))
    ...
    ## Platy WorldBuilder ##
        ...
        if iPlayer > -1:
            iUnit = -1
            # Religionen
            if iTechType == gc.getInfoTypeForString("TECH_RELIGION_NORDIC"):
                iUnit = gc.getInfoTypeForString("UNIT_NORDIC_MISSIONARY")
            elif iTechType == gc.getInfoTypeForString("TECH_RELIGION_CELTIC"):
                iUnit = gc.getInfoTypeForString("UNIT_CELTIC_MISSIONARY")
            elif iTechType == gc.getInfoTypeForString("TECH_RELIGION_SUMER"):
                iUnit = gc.getInfoTypeForString("UNIT_SUMER_MISSIONARY")
            elif iTechType == gc.getInfoTypeForString("TECH_RELIGION_GREEK"):
                iUnit = gc.getInfoTypeForString("UNIT_GREEK_MISSIONARY")
            elif iTechType == gc.getInfoTypeForString("TECH_RELIGION_EGYPT"):
                iUnit = gc.getInfoTypeForString("UNIT_EGYPT_MISSIONARY")
            elif iTechType == gc.getInfoTypeForString("TECH_RELIGION_PHOEN"):
                iUnit = gc.getInfoTypeForString("UNIT_PHOEN_MISSIONARY")
            elif iTechType == gc.getInfoTypeForString("TECH_RELIGION_HINDU"):
                iUnit = gc.getInfoTypeForString("UNIT_HINDU_MISSIONARY")
            elif iTechType == gc.getInfoTypeForString("TECH_RELIGION_ROME"):
                iUnit = gc.getInfoTypeForString("UNIT_ROME_MISSIONARY")
            elif iTechType == gc.getInfoTypeForString("TECH_DUALISMUS"):
                iUnit = gc.getInfoTypeForString("UNIT_ZORO_MISSIONARY")
            # Kulte
            elif iTechType == gc.getInfoTypeForString("TECH_FRUCHTBARKEIT"):
                iUnit = gc.getInfoTypeForString("UNIT_EXECUTIVE_2")
            elif iTechType == gc.getInfoTypeForString("TECH_MANUFAKTUREN"):
                iUnit = gc.getInfoTypeForString("UNIT_EXECUTIVE_3")
            elif iTechType == gc.getInfoTypeForString("TECH_GLADIATOR"):
                iUnit = gc.getInfoTypeForString("UNIT_EXECUTIVE_5")
            ...
            if iUnit > -1:
                ...
                lCities = PyPlayer(iPlayer).getCityList()
                if len(lCities) > 0:
                    iRand = CvUtil.myRandom(len(lCities))
...
#    if pCity.canConscript():
        # Seek Plots
            ...
            if len(rebelPlotArray) > 0:
                ...
                # Number of Partisans
                iAnzahl = CvUtil.myRandom(pCity.getPopulation()) + 1
...
# PAE triumph movies when city is reconquered
        if gc.getPlayer(iNewOwner).isHuman():
            if pCity.getOriginalOwner() == iNewOwner:
                ...
                if gc.getPlayer(iNewOwner).getCurrentEra() > 2:
                    iVids = 3
                elif gc.getPlayer(iNewOwner).getCurrentEra() > 1:
                    iVids = 2
                else:
                    iVids = 1
                # GG dying movies starts at no 3 (CvWonderMovieScreen)
                iMovie = 1 + CvUtil.myRandom(iVids)
                ...
                for i in range(iCityGP1):
                    ...
                    if iRand < 5:
                        ...
                        if gc.getPlayer(iNewOwner).isHuman():
                            ...
                            if iRand == 0:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_CATCH_GP1_1", (0, 0))
                            elif iRand == 1:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_CATCH_GP1_2", (0, 0))
                            elif iRand == 2:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_CATCH_GP1_3", (0, 0))
                        ...
                        if gc.getPlayer(iNewOwner).isHuman():
                            iRand = CvUtil.myRandom(3)
                            if iRand == 0:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_FLEE_GP1_1", (0, 0))
                            elif iRand == 1:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_FLEE_GP1_2", (0, 0))
                            elif iRand == 2:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_FLEE_GP1_3", (0, 0))
                        ...
                        iRand = CvUtil.myRandom(3)
                        if iRand == 0:
                            text = CyTranslator().getText("TXT_KEY_MESSAGE_UNCATCH_GP1_1", (0, 0))
                        elif iRand == 1:
                            text = CyTranslator().getText("TXT_KEY_MESSAGE_UNCATCH_GP1_2", (0, 0))
                        elif iRand == 2:
                            text = CyTranslator().getText("TXT_KEY_MESSAGE_UNCATCH_GP1_3", (0, 0))
                        CyInterface().addMessage(iNewOwner, True, 10, text, None, 2, None, ColorTypes(7), 0, 0, False, False)
            # Artist
            if iCityGP2 > 0:
                ...
                for i in range(iCityGP2):
                    ...
                    if iRand < 5:
                        ...
                        if gc.getPlayer(iNewOwner).isHuman():
                            iRand = CvUtil.myRandom(3)
                            if iRand == 0:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_CATCH_GP2_1", (0, 0))
                            elif iRand == 1:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_CATCH_GP2_2", (0, 0))
                            elif iRand == 2:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_CATCH_GP2_3", (0, 0))
                            CyInterface().addMessage(iNewOwner, True, 10, text, None, 2, None, ColorTypes(14), 0, 0, False, False)
                    elif iRand < 9 and gc.getPlayer(iPreviousOwner).isAlive():
                        iJump2Plot = CvUtil.myRandom(len(fleePlotArray))
                        pUnit = gc.getPlayer(iPreviousOwner).initUnit(iNewUnit, fleePlotArray[iJump2Plot].getX(), fleePlotArray[iJump2Plot].getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
                        ...
                        if gc.getPlayer(iNewOwner).isHuman():
                            iRand = CvUtil.myRandom(3)
                            if iRand == 0:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_FLEE_GP2_1", (0, 0))
                            elif iRand == 1:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_FLEE_GP2_2", (0, 0))
                            elif iRand == 2:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_FLEE_GP2_3", (0, 0))
                            CyInterface().addMessage(iNewOwner, True, 10, text, None, 2, None, ColorTypes(7), 0, 0, False, False)
                    elif gc.getPlayer(iNewOwner).isHuman():
                        iRand = CvUtil.myRandom(3)
                        if iRand == 0:
                            text = CyTranslator().getText("TXT_KEY_MESSAGE_UNCATCH_GP2_1", (0, 0))
                        elif iRand == 1:
                            text = CyTranslator().getText("TXT_KEY_MESSAGE_UNCATCH_GP2_2", (0, 0))
                        elif iRand == 2:
                            text = CyTranslator().getText("TXT_KEY_MESSAGE_UNCATCH_GP2_3", (0, 0))
                        CyInterface().addMessage(iNewOwner, True, 10, text, None, 2, None, ColorTypes(7), 0, 0, False, False)
            # Scientist
            if iCityGP3 > 0:
                ...
                for i in range(iCityGP3):
                    ...
                    if iRand < 5:
                        ...
                        if gc.getPlayer(iNewOwner).isHuman():
                            iRand = CvUtil.myRandom(3)
                            if iRand == 0:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_CATCH_GP3_1", (0, 0))
                            elif iRand == 1:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_CATCH_GP3_2", (0, 0))
                            elif iRand == 2:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_CATCH_GP3_3", (0, 0))
                            CyInterface().addMessage(iNewOwner, True, 10, text, None, 2, None, ColorTypes(14), 0, 0, False, False)
                    elif iRand < 9 and gc.getPlayer(iPreviousOwner).isAlive():
                        iJump2Plot = CvUtil.myRandom(len(fleePlotArray))
                        pUnit = gc.getPlayer(iPreviousOwner).initUnit(iNewUnit, fleePlotArray[iJump2Plot].getX(), fleePlotArray[iJump2Plot].getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
                        ...
                        if gc.getPlayer(iNewOwner).isHuman():
                            iRand = CvUtil.myRandom(3)
                            if iRand == 0:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_FLEE_GP3_1", (0, 0))
                            elif iRand == 1:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_FLEE_GP3_2", (0, 0))
                            elif iRand == 2:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_FLEE_GP3_3", (0, 0))
                            CyInterface().addMessage(iNewOwner, True, 10, text, None, 2, None, ColorTypes(7), 0, 0, False, False)
                    elif gc.getPlayer(iNewOwner).isHuman():
                        iRand = CvUtil.myRandom(3)
                        if iRand == 0:
                            text = CyTranslator().getText("TXT_KEY_MESSAGE_UNCATCH_GP3_1", (0, 0))
                        elif iRand == 1:
                            text = CyTranslator().getText("TXT_KEY_MESSAGE_UNCATCH_GP3_2", (0, 0))
                        elif iRand == 2:
                            text = CyTranslator().getText("TXT_KEY_MESSAGE_UNCATCH_GP3_3", (0, 0))
                        CyInterface().addMessage(iNewOwner, True, 10, text, None, 2, None, ColorTypes(7), 0, 0, False, False)
            # Merchant
            if iCityGP4 > 0:
                ...
                for i in range(iCityGP4):
                    ...
                    if iRand < 5:
                        ...
                        if gc.getPlayer(iNewOwner).isHuman():
                            iRand = CvUtil.myRandom(3)
                            if iRand == 0:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_CATCH_GP4_1", (0, 0))
                            elif iRand == 1:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_CATCH_GP4_2", (0, 0))
                            elif iRand == 2:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_CATCH_GP4_3", (0, 0))
                            CyInterface().addMessage(iNewOwner, True, 10, text, None, 2, None, ColorTypes(14), 0, 0, False, False)
                    elif iRand < 9 and gc.getPlayer(iPreviousOwner).isAlive():
                        iJump2Plot = CvUtil.myRandom(len(fleePlotArray))
                        pUnit = gc.getPlayer(iPreviousOwner).initUnit(iNewUnit, fleePlotArray[iJump2Plot].getX(), fleePlotArray[iJump2Plot].getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
                        ...
                        if gc.getPlayer(iNewOwner).isHuman():
                            iRand = CvUtil.myRandom(3)
                            if iRand == 0:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_FLEE_GP4_1", (0, 0))
                            elif iRand == 1:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_FLEE_GP4_2", (0, 0))
                            elif iRand == 2:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_FLEE_GP4_3", (0, 0))
                            CyInterface().addMessage(iNewOwner, True, 10, text, None, 2, None, ColorTypes(7), 0, 0, False, False)
                    elif gc.getPlayer(iNewOwner).isHuman():
                        iRand = CvUtil.myRandom(3)
                        if iRand == 0:
                            text = CyTranslator().getText("TXT_KEY_MESSAGE_UNCATCH_GP4_1", (0, 0))
                        elif iRand == 1:
                            text = CyTranslator().getText("TXT_KEY_MESSAGE_UNCATCH_GP4_2", (0, 0))
                        elif iRand == 2:
                            text = CyTranslator().getText("TXT_KEY_MESSAGE_UNCATCH_GP4_3", (0, 0))
                        CyInterface().addMessage(iNewOwner, True, 10, text, None, 2, None, ColorTypes(7), 0, 0, False, False)
            # Engineer
            if iCityGP5 > 0:
                ...
                for i in range(iCityGP5):
                    ...
                    if iRand < 5:
                        ...
                        if gc.getPlayer(iNewOwner).isHuman():
                            iRand = CvUtil.myRandom(3)
                            if iRand == 0:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_CATCH_GP5_1", (0, 0))
                            elif iRand == 1:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_CATCH_GP5_2", (0, 0))
                            elif iRand == 2:
                                text = CyTranslator().getText("TXT_KEY_MESSAGE_CATCH_GP5_3", (0, 0))
                            CyInterface().addMessage(iNewOwner, True, 10, text, None, 2, None, ColorTypes(14), 0, 0, False, False)
                    elif iRand < 9 and gc.getPlayer(iPreviousOwner).isAlive():
                        iJump2Plot = CvUtil.myRandom(len(fleePlotArray))
                        pUnit = gc.getPlayer(iPreviousOwner).initUnit(iNewUnit, fleePlotArray[iJump2Plot].getX(), fleePlotArray[iJump2Plot].getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
...
# --- PAE V Patch4: nur ab Pop 3 (sonst exploit)
            if bConquest and (bAssimilation or pCity.getPopulation() > 2):
                ...
                if iRand == 1 or bGetTech:
                    ...
                    if len(TechArray) > 0:
                        bGetTech = True
                        iTechRand = CvUtil.myRandom(len(TechArray))
                        iTech = TechArray[iTechRand]
                        ...
                        if gc.getPlayer(iNewOwner).isHuman():
                            ...
                            gc.getPlayer(iNewOwner).clearResearchQueue()
                ...
                if not bGetTech:
                    ...
                    if iTechNew != -1:
                        ...
                        if pOldOwner.getTechScore() < pNewOwner.getTechScore():
...
#    CvUtil.pyPrint("%s has grown to size %i" %(pCity.getName(),pCity.getPopulation()))
    ...
    def onCityDoTurn(self, argsList):
        ...
        if pTeam.isHasTech(iTech) and popCity > 3:
            PAE_City.doCreateEmigrant(pCity)
