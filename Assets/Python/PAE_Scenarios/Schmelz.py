...
def onLoadGame(sScenarioName):
  ...
  for x in xrange(CyMap().getGridWidth()):
    for y in xrange(CyMap().getGridHeight()):
      ...
      elif pPlot.getTerrainType() == eSnow: IceSnow.append(pPlot)
      elif pPlot.getTerrainType() == eTundra:
        # Extra Tundra-Bereich, wos langsamer gehn soll
        if sScenarioName == "SchmelzEuro" and x >= 42 and y >= 49: IceTundra2.append(pPlot)
        else: IceTundra.append(pPlot)
      elif pPlot.getFeatureType() == eEis: IceEis.append(pPlot)
      #Ueberflutung
      if pPlot.isCoastalLand() and not pPlot.isCity() and not pPlot.isHills() and not pPlot.isPeak() and pPlot.getBonusType(pPlot.getOwner()) == -1:
        if sScenarioName == "SchmelzEuro":
          if y >= 28: IceCoast.append(pPlot)
        else: IceCoast.append(pPlot)
      #Desertifizierung
      if sScenarioName == "SchmelzEuro" and y <= 8:
        if pPlot.getTerrainType() == eEbene and not pPlot.isCoastalLand() and not pPlot.isCity() and not pPlot.isRiver(): IceDesertEbene.append(pPlot)
        elif pPlot.getTerrainType() == eCoast and y <=6 and x >= 2 and x <= 60: IceDesertCoast.append(pPlot)
  ...
  if iGameTurn == 1:
    for x in xrange(CyMap().getGridWidth()):
      for y in xrange(CyMap().getGridHeight()):
        pPlot = CyMap().plot(x,y)
        if pPlot.getTerrainType() == eDarkIce: continue
        if pPlot.getTerrainType() == eSnow: IceSnow.append(pPlot)
        elif pPlot.getTerrainType() == eTundra:
          # Extra Tundra-Bereich, wos langsamer gehn soll
          if sScenarioName == "SchmelzEuro" and x >= 42 and y >= 49: IceTundra2.append(pPlot)
          else: IceTundra.append(pPlot)
    ...
    if iGameTurn % iTurnSchmelzIntervall == 0 and iGameTurn <= iTurnLimit:
      ...
      if len(IceSnow):
        for pPlot in IceSnow:
          iRand = myRandom(iWahrscheinlichkeit)
          if iRand == 1:
            pPlot.setTerrainType(eTundra,1,1)
            IceSnow.remove(pPlot)
            # Tundra Liste updaten
            if sScenarioName == "SchmelzEuro" and pPlot.getX() >= 42 and pPlot.getY() >= 49: IceTundra2.append(pPlot)
            else: IceTundra.append(pPlot)
      ...
      if len(IceTundra2):
        for pPlot in IceTundra2:
          iRand = myRandom(iWahrscheinlichkeit*3)
          if iRand == 1:
            iRand = myRandom(4)
            if iRand == 1: pPlot.setTerrainType(eGras,1,1)
            else: pPlot.setTerrainType(eEbene,1,1)
