...
def changeBuildingHappyChange(pCity, iBuilding, iChange):
    ...
    pCity.setBuildingHappyChange(
        eBuildingClass,
        pCity.getBuildingHappyChange(eBuildingClass) +
        iChange)

# Returns whether pCity has access to eBonus, ignoring free bonuses (from
# trade). Gets an own function bc. it used several times.
...
def getScoreComponent(
    ...
    if bFinal:
        iScore = ((100 + gc.getDefineINT("SCORE_HANDICAP_PERCENT_OFFSET") +
                   (gc.getGame().getHandicapType() * gc.getDefineINT(
                        "SCORE_HANDICAP_PERCENT_PER"))) * iScore) / 100
...
def decode_script_data(sData):
    try:
        data = json.loads(sData, object_hook=_decode_dict)
    except ValueError:
        if len(sData) > 0:
            return {"t": sData}
        else:
...
def setScriptData(pOwner, sd):
    ...
    if isinstance(sd, dict):
        ...
        pOwner.setScriptData(encode_script_data({"t":str(sd)}) )
    """
