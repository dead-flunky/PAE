        ...
        if pHeadSelectedUnit is not None and not pHeadSelectedUnit.isNone():
            ...
            if iUnitType in PAE_Cultivation.lCultivationUnits:
                ...
                if eBonus != -1 and not gc.getActivePlayer().isOption(PlayerOptionTypes.PLAYEROPTION_NO_UNIT_RECOMMENDATIONS):
                    ...
                    (pCity, pIter) = pPlayer.firstCity(False)
                    while pCity:
                        #pCity = pHeadSelectedUnit.plot().getWorkingCity()
                        if pCity is not None:
                            pBestPlot = PAE_Cultivation.AI_bestCultivation(pCity, 0, eBonus)
                            if pBestPlot:
                                ...
                                pSecondBestPlot = PAE_Cultivation.AI_bestCultivation(pCity, 1, eBonus)
                                if pSecondBestPlot:
                        ...
                        (pCity, pIter) = pPlayer.nextCity(pIter, False)
    ...
    def AI_chooseProduction(self, argsList):
        ...
        if pCity.canCreate(iProjectX, 0, 0) and not pCity.isProductionProject():
            ...
            if pCity.hasBonus(iBonus):
                iProject = iProjectX
            elif gc.getTeam(pPlayer.getTeam()).isHasTech(gc.getInfoTypeForString("TECH_DEFENCES")):
                iProject = iProjectX
        ...
        if pCity.canCreate(iProjectX, 0, 0) and not pCity.isProductionProject():
            ...
            for pyCity in lCities:
                if pyCity.hasReligion(iReligion):
                    iNumCitiesWithChristianity += 1

            if iNumCitiesWithChristianity > len(lCities) / 2:
                iProject = iProjectX
        ...
        if pCity.getPopulation() > 4:
            ...
            if pCity.getPopulation() > 5:
                if pPlayer.getGold() < 500:
                    if pPlayer.calculateGoldRate() < 5:
                        if 1 == CvUtil.myRandom(2, "ai_build_merchant"):
            ...
            if iRand < 2:
                ...
                if pCity.canTrain(Unit1, 0, 0) or pCity.canTrain(Unit2, 0, 0) or pCity.canTrain(Unit3, 0, 0) or pCity.canTrain(Unit4, 0, 0) or pCity.canTrain(Unit5, 0, 0) \
                    ...
                    if Unit1 in lUnitTypes:
                        bUnit1 = True
                    if Unit2 in lUnitTypes:
                        bUnit2 = True
                    if Unit3 in lUnitTypes:
                        bUnit3 = True
                    if Unit4 in lUnitTypes:
                        bUnit4 = True
                    if Unit5 in lUnitTypes:
                        bUnit5 = True
                    if Unit6 in lUnitTypes:
                        bUnit6 = True
                    if Unit7 in lUnitTypes:
                        bUnit7 = True
                    ...
                    for lCity in lCities:
                        ...
                        if pCityX.isProductionUnit():
                            if not bUnit1:
                                if pCityX.getUnitProduction(Unit1):
                                    bUnit1 = True
                            if not bUnit2:
                                if pCityX.getUnitProduction(Unit2):
                                    bUnit2 = True
                            if not bUnit3:
                                if pCityX.getUnitProduction(Unit3):
                                    bUnit3 = True
                            if not bUnit4:
                                if pCityX.getUnitProduction(Unit4):
                                    bUnit4 = True
                            if not bUnit5:
                                if pCityX.getUnitProduction(Unit5):
                                    bUnit5 = True
                            if not bUnit6:
                                if pCityX.getUnitProduction(Unit6):
                                    bUnit6 = True
                            if not bUnit7:
                                if pCityX.getUnitProduction(Unit7):
                                    bUnit7 = True
...
# und weiter unten aendern!
            ...
            # Ab Veteran Status
                ...
                elif 1 == CvUtil.myRandom(20, "ai_vet_res"):
                ...
                if pUnit.getGroup().getLengthMissionQueue() == 0:
                    if pOwner.getNumCities() > 0:
                        ...
                        if pCapital.getID() != -1 and pPlot.getArea() == pCapital.area().getID():
                            pCityX = pCapital
                            if not pCapital.isNone() and pCapital is not None:
