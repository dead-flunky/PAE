...
"""
NUM_CIV_PLAYERS = gc.getMAX_CIV_PLAYERS()  # 18 or 40
Replace_barb_id = False  # Could be changed during WB Save reading
Barb_id = 18  # No constant value. Could be changed during savegame loading. (40er DLL)
Id18WasBarbarian = False  # Could be changed during savegame loading. (40er DLL)


def filterPlayerIds(list_of_ids):
    if Replace_barb_id:
        l = [x for x in list_of_ids if x < NUM_CIV_PLAYERS]
        if list_of_ids[-1] == Barb_id:
            l.append(NUM_CIV_PLAYERS)
    else:
        return [x for x in list_of_ids if x <= NUM_CIV_PLAYERS]

# Same as above but compare list_of_ids[0]


def filterPlayerIds2(list_of_ids):
    if Replace_barb_id:
        l = [x for x in list_of_ids if x[0] < NUM_CIV_PLAYERS]
        if list_of_ids[-1][0] == Barb_id:
            b = [NUM_CIV_PLAYERS]
            b.extend(list_of_ids[-1][1:])
            l.append(b)
    else:
        return [x for x in list_of_ids if x[0] <= NUM_CIV_PLAYERS]
...
class CvCityDesc:
    ...
    def apply(self):
        ...
        self.lCulture = filterPlayerIds2(self.lCulture)
...
class CvPlotDesc:
    ...
    def apply(self):
        ...
        #self.abTeamPlotRevealed = filterPlayerIds(self.abTeamPlotRevealed)
        #self.lCulture = filterPlayerIds2(self.lCulture)
...
class CvMapDesc:
    ...
    def write(self, f):
        ...
        f.write("\ttop latitude=%d\n" % (cymap.getTopLatitude(),))
        f.write("\tbottom latitude=%d\n" % (cymap.getBottomLatitude(),))
        f.write("\twrap X=%d\n" % (cymap.isWrapX(),))
        f.write("\twrap Y=%d\n" % (cymap.isWrapY(),))
        f.write("\tworld size=%s\n" % (gc.getWorldInfo(cymap.getWorldSize()).getType(),))
        f.write("\tclimate=%s\n" % (gc.getClimateInfo(cymap.getClimate()).getType(),))
        f.write("\tsealevel=%s\n" % (gc.getSeaLevelInfo(cymap.getSeaLevel()).getType(),))
...
## Platy Builder ##
    ...
    def applyInitialItems(self):
        for iTeamLoop in xrange(len(self.teamsDesc)):
            ...
            # PAE, adapt player ids in arrays
            pWBTeam.aaiEspionageAgainstTeams = filterPlayerIds2(pWBTeam.aaiEspionageAgainstTeams)
            pWBTeam.bContactWithTeamList = filterPlayerIds(pWBTeam.bContactWithTeamList)
            pWBTeam.bWarWithTeamList = filterPlayerIds(pWBTeam.bWarWithTeamList)
            pWBTeam.bPermanentWarPeaceList = filterPlayerIds(pWBTeam.bPermanentWarPeaceList)
            pWBTeam.bOpenBordersWithTeamList = filterPlayerIds(pWBTeam.bOpenBordersWithTeamList)
            pWBTeam.bDefensivePactWithTeamList = filterPlayerIds(pWBTeam.bDefensivePactWithTeamList)
            pWBTeam.bVassalOfTeamList = filterPlayerIds(pWBTeam.bVassalOfTeamList)
    ...
    ## Platy Builder ##
        for iPlayerLoop in xrange(len(self.playersDesc)):
            ...
            pWBPlayer.aaiAttitudeExtras = filterPlayerIds2(pWBPlayer.aaiAttitudeExtras)
    ...
    ## Platy Builder ##
        for iPlotLoop in range(self.mapDesc.numPlotsWritten):
            ...
            pWBPlot.lCulture = filterPlayerIds2(pWBPlot.lCulture)
        ...
        for iPlotLoop in range(self.mapDesc.numPlotsWritten):
            pWBPlot = self.plotDesc[iPlotLoop]
            pWBPlot.abTeamPlotRevealed = filterPlayerIds(pWBPlot.abTeamPlotRevealed)
    ...
    ## Platy Builder ##
        ...
        # PAE, Extend to 40 entries regardless if they will be used
        iNumPlayers2 = gc.getMAX_PLAYERS()  # 19 or 41; similar to iNumPlayers if Platy Builder, otherwise iNumPlayers+1
        iNumTeams2 = gc.getMAX_TEAMS()
        if numTeams < iNumTeams2:
            for i in range(numTeams, iNumTeams2):
                teamDesc = CvTeamDesc()
                self.teamsDesc.append(teamDesc)
        ...
        # PAE; If the WB save was written with the 40er DLL the Barbarian entry is != 18, but 40.
        ...
        while True:
            filePos = f.tell()
            playerDesc = CvPlayerDesc()
            if playerDesc.read(f) == True:
                barbarianDesc = playerDesc
                continue
            # Last slot was already passed. Abort and move up
            f.seek(filePos)
            break
        ...
        if barbarianDesc is not None:
            if(len(self.playersDesc) == iNumPlayers2):
                ...
                self.playersDesc.append(barbarianDesc)
                numPlayers += 1

        # PAE Expand to 40 entries if extended DLL is used
        if numPlayers < iNumPlayers2:
            for i in range(numPlayers, iNumPlayers2):
                playerDesc = CvPlayerDesc()
                #playerDesc.team = i
            ...
            barbTeam = self.playersDesc[18].team
            teamDesc = CvTeamDesc()
            self.teamsDesc[barbTeam] = teamDesc
            playerDesc = CvPlayerDesc()
            playerDesc.team = barbTeam
            self.playersDesc[18] = playerDesc

        # Check if too many players was loaded
        if numPlayers > iNumPlayers2:
            global Replace_barb_id
            global Barb_id
            Replace_barb_id = True
            Barb_id = iNumPlayers2-1

        # PAE, if player 18 is Barbarian, swap this slot with player 40.
        # We assume here that non-barb teams has no 'AtWar' and 'ContactWithTeam' entries for barbarian teams.
        if numPlayers > 19 and self.playersDesc[18].leaderType == "LEADER_BARBARIAN":
            if self.playersDesc[iNumPlayers2-1].leaderType == "LEADER_BARBARIAN":
                ...
                # Swap barb to from position 18 to 40
                barbTeam = self.playersDesc[18].team
                self.teamsDesc[barbTeam], self.teamsDesc[iNumTeams-1] = self.teamsDesc[iNumTeams-1], self.teamsDesc[barbTeam]
                self.playersDesc[18].team = iNumTeams-1
#       self.playersDesc[iNumPlayers2-1].team = barbTeam
                self.playersDesc[18], self.playersDesc[iNumPlayers2-1] = self.playersDesc[iNumPlayers2-1], self.playersDesc[18]
