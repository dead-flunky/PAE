            ...
            if (pHeadSelectedCity.getOwner() == gc.getGame().getActivePlayer()) or gc.getGame().isDebugMode():
                ...
                for i in range(g_NumBuildingClassInfos):
                    if not isLimitedWonderClass(i):
                        ...
                        if pHeadSelectedCity.canConstruct(eLoopBuilding, False, True, False):
                            ...
                            self.iconsRight[rowRight].append(([szButton, WidgetTypes.WIDGET_CONSTRUCT, i, -1, False], pHeadSelectedCity.canConstruct(eLoopBuilding, False, False, False), cityTab))
                            iCount = iCount + 1
                            bFound = True
                ...
                for i in range(g_NumProjectInfos):
                    if pHeadSelectedCity.canCreate(i, False, True):
                        ...
                        self.iconsRight[rowRight].append(([szButton, WidgetTypes.WIDGET_CREATE, i, -1, False], pHeadSelectedCity.canCreate(i, False, False), cityTab))
                        iCount = iCount + 1
                        bFound = True
                ...
                for i in range(g_NumProcessInfos):
                    if pHeadSelectedCity.canMaintain(i, False):
                        ...
                        self.iconsRight[rowRight].append(([szButton, WidgetTypes.WIDGET_MAINTAIN, i, -1, False], True, cityTab))
                        iCount = iCount + 1
                        bFound = True
