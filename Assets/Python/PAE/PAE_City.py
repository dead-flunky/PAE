...
def doCheckGlobalBuilding(iPlayer, iBuilding):
    ...
    (city, pIter) = pPlayer.firstCity(False)
    if city is not None and not city.isNone():
        ...
        while city:
            ...
            (city, pIter) = pPlayer.nextCity(pIter, False)
...
def doCreateEmigrant(pCity):
    ...
    if not bRevoltDanger:
        bRevoltDanger = pPlayer.getAnarchyTurns() > 0

    if not bRevoltDanger:
        iRel = pPlayer.getStateReligion()
        bRevoltDanger = iRel != -1 and not pCity.isHasReligion(iRel)

    if not bRevoltDanger:
        if pPlayer.getCommercePercent(0) > 50:
            iChance = int((pPlayer.getCommercePercent(0) - 50) / 5)
            # Pro Happy Citizen 5% Nachlass
            iChance = iChance - pCity.happyLevel() + pCity.unhappyLevel(0)
    ...
    if bRevoltDanger:
        if CvUtil.myRandom(100) < iChance:
            ...
            if iPlayerCulture == -1 or iPlayerCulture == "NO_PLAYER":
