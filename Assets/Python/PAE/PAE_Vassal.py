    ...
    if pWinnerTeam.isHasTech(iTechVasallentum) and iNewOwner != gc.getBARBARIAN_PLAYER():
        ...
        for iVassal in range(iRange):
            ...
            if pPlayer.isAlive():
                ...
                if pTeam.isVassal(iLoserTeam):
                    ...
                    if iVassalPower + iWinnerPower > iLoserPower and (CvUtil.myRandom(30) + pPlayer.AI_getAttitude(iPreviousOwner) - pPlayer.AI_getAttitude(iNewOwner)) < 10:
                        ...
                        # ------------------------------
                            ...
                            if bDeclareWar:
                                ...
                                if pWinner.isHuman():
                                    ...
                                    popupInfo.setText(
                                        CyTranslator().getText("TXT_KEY_POPUP_VASSAL_09_1",(pLoser.getCivilizationAdjective(3),gc.getLeaderHeadInfo(pLoser.getLeaderType()).getDescription(),)) )
                                    popupInfo.addPopup(iNewOwner)
        ...
        if iLoserPowerWithVassals < iWinnerPower and pLoser.getNumCities() > 0 and pLoser.getNumCities() < iMinimumCities and iNewOwner != gc.getBARBARIAN_PLAYER() and iPreviousOwner != gc.getBARBARIAN_PLAYER():
            ...
            # KI bietet der HI Vasallenstatus an, 120% - 10% pro Stadt
                ...
                if 12 - pLoser.getNumCities() > CvUtil.myRandom(10):
                    iGold = pLoser.getGold() / 2 + CvUtil.myRandom(pLoser.getGold() / 2)
                    iGold = int(iGold)
                    popupInfo = CyPopupInfo()
                    popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON)
        ...
        elif pLoser.isHuman() and iLoserPowerWithVassals <= iWinnerPower*1.2:
            iGold = pLoser.getGold() / 2 + CvUtil.myRandom(pLoser.getGold() / 2)
            iGold = int(iGold)
            popupInfo = CyPopupInfo()
            popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON)
...
def VassalHItoHI (iNewOwner, iPreviousOwner, pCity):
    ...
    iGold1 = CvUtil.myRandom(pLoser.getGold() / 2)
...
def do682(iWinner, iLoser, iGold):
    ...
    if iGold == -1:
        iGold1 = CvUtil.myRandom(pWinner.getGold() / 4)
