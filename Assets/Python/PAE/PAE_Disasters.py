...
#  5 = WORLDSIZE_HUGE
    ...
    for _ in range(20):
        iRandX = CvUtil.myRandom(iMapW)
        iRandY = CvUtil.myRandom(iMapH)
...
#  5 = WORLDSIZE_HUGE
    ...
    for _ in range(10):
        ...
        if pPlot is not None and not pPlot.isNone():
            if pPlot.getFeatureType() == iDarkIce:
                continue
            if pPlot.getFeatureType() not in lFeatures and not pPlot.isPeak() and (pPlot.getTerrainType() == terr_desert or pPlot.getTerrainType() == terr_plains):
                OwnerArray = []
                iMaxEffect += 1
                for i in range(3):
                    for j in range(5):
                        ...
                        if iSetStorm == 1:
                            loopPlot = gc.getMap().plot(pPlot.getX() + i - 1, pPlot.getY() + j - 2)
                            if loopPlot is not None and not loopPlot.isNone():
                                if not loopPlot.isWater() and not loopPlot.isPeak() and loopPlot.getFeatureType() not in lFeatures \
...
def doErdbeben(iX, iY):
    ...
    if iX > 0 and iY > 0:
        ...
        iSkala = 6 + CvUtil.myRandom(4)

        iMapW = gc.getMap().getGridWidth()
        iMapH = gc.getMap().getGridHeight()

        # Plot soll nicht ganz am Rand sein (Flunky: alle 4 Raender ausnehmen)
        iRandX = 3 + CvUtil.myRandom(iMapW - 6)
        iRandY = 3 + CvUtil.myRandom(iMapH - 6)
        pPlot = gc.getMap().plot(iRandX, iRandY)
        iPlayer = pPlot.getOwner()
        ...
        if pPlot is not None and not pPlot.isNone():
            if not pPlot.isWater() and pPlot.getFeatureType() != iDarkIce:
                ...
                if bonusPlotArray and CvUtil.myRandom(10) < 2:
                    lBonus = [
                    ...
                    iRand = CvUtil.myRandom(len(lBonus))
                    iBonus = lBonus[iRand]

                    iRandPlot = CvUtil.myRandom(len(bonusPlotArray))
        ...
        if pPlot is not None and not pPlot.isNone():
            ...
            for i in range(3):
                for j in range(3):
                    loopPlot = gc.getMap().plot(iRandX - 1 + i, iRandY - 1 + j)
                    if loopPlot is not None and not loopPlot.isNone():
                        if loopPlot.getFeatureType() != feat_flood_plains and not loopPlot.isPeak() and not loopPlot.isWater() and loopPlot.getFeatureType() != feat_oasis:
                            if loopPlot.getFeatureType() == feat_forest or loopPlot.getFeatureType() == feat_forest2:
                                loopPlot.setFeatureType(feat_forest_burnt, 0)
                            elif loopPlot.getFeatureType() != feat_ice and loopPlot.getTerrainType() != terr_snow:
                                loopPlot.setFeatureType(feat_brand, 0)
                        ...
                        if CvUtil.myRandom(2) == 1 and not loopPlot.isCity():
                            loopPlot.setRouteType(-1)
                            loopPlot.setImprovementType(-1)
                        doKillUnits(loopPlot, 20)
...
def doDestroyCityWonders(pCity, iChance, iFeatureType):
    if pCity.getNumBuildings() > 0:
        ...
        for iBuilding in range(iRange):
            pBuilding = gc.getBuildingInfo(iBuilding)
            if pCity.getNumBuilding(iBuilding) and isWorldWonderClass(gc.getBuildingInfo(iBuilding).getBuildingClassType()) and CvUtil.myRandom(100, "destroyWW") < iChance:
                pCity.setNumRealBuilding(iBuilding, 0)
                # Messages
                pOwner = gc.getPlayer(iOwner)
...
def doDestroyWalls(pCity):
    ...
    if pCity.isHasBuilding(iBuildingWalls) and CvUtil.myRandom(100, "destroy_walls") < 50:
        pBuilding = gc.getBuildingInfo(iBuildingWalls)
        pCity.setNumRealBuilding(iBuildingWalls, 0)
        if iPlayer != -1 and gc.getPlayer(iPlayer).isHuman():
            CyInterface().addMessage(gc.getPlayer(iPlayer).getID(), True, 8, CyTranslator().getText("TXT_KEY_DISASTER_DESTROYED_BUILDING", (pCity.getName(), pBuilding.getDescription())), None, 2, pBuilding.getButton(), ColorTypes(7), pCity.getX(), pCity.getY(), True, True)

    if pCity.isHasBuilding(iBuildingHW1) and CvUtil.myRandom(100, "destroy_HW1") < 20:
        pBuilding = gc.getBuildingInfo(iBuildingHW1)
        pCity.setNumRealBuilding(iBuildingHW1, 0)
        if iPlayer != -1 and gc.getPlayer(iPlayer).isHuman():
            CyInterface().addMessage(gc.getPlayer(iPlayer).getID(), True, 8, CyTranslator().getText("TXT_KEY_DISASTER_DESTROYED_BUILDING", (pCity.getName(), pBuilding.getDescription())), None, 2, pBuilding.getButton(), ColorTypes(7), pCity.getX(), pCity.getY(), True, True)

    if pCity.isHasBuilding(iBuildingHW2) and CvUtil.myRandom(100, "destroy_HW2") < 20:
        pBuilding = gc.getBuildingInfo(iBuildingHW2)
        pCity.setNumRealBuilding(iBuildingHW2, 0)
        if iPlayer != -1 and gc.getPlayer(iPlayer).isHuman():
            CyInterface().addMessage(gc.getPlayer(iPlayer).getID(), True, 8, CyTranslator().getText("TXT_KEY_DISASTER_DESTROYED_BUILDING", (pCity.getName(), pBuilding.getDescription())), None, 2, pBuilding.getButton(), ColorTypes(7), pCity.getX(), pCity.getY(), True, True)

    if pCity.isHasBuilding(iBuildingHW3) and CvUtil.myRandom(100, "destroy_HW3") < 20:
