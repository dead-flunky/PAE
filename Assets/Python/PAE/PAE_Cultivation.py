...
def _getBonusCultivationChance(iPlayer, pPlot, eBonus, bVisibleOnly=True):
    ...
    if eBonus in lCorn and not pPlot.isFreshWater():
        ...
        if eBonus == gc.getInfoTypeForString("BONUS_OLIVES") and not pPlot.isCoastalLand():
            ...
            return 0
        if eBonus == gc.getInfoTypeForString("BONUS_GRAPES") and not pPlot.isFreshWater():
...
def getCityCultivationAmount(pCity):
    iAnz = 1
    if pCity.isHasBuilding(gc.getInfoTypeForString("BUILDING_STADT")): iAnz += 1
    if pCity.isHasBuilding(gc.getInfoTypeForString("BUILDING_PROVINZ")): iAnz += 1
    if pCity.isHasBuilding(gc.getInfoTypeForString("BUILDING_METROPOLE")): iAnz += 1
    return iAnz
...
def doCultivateBonus(pPlot, pUnit, eBonus):
    ...
    bOnlyVisible = False
    iChance = _getBonusCultivationChance(iPlayer, pPlot, eBonus, bOnlyVisible)
...
def getCityCultivationPlot(pCity, eBonus):
    ...
    for iI in range(gc.getNUM_CITY_PLOTS()):
        ...
        if pLoopPlot != None and not pLoopPlot.isNone():
            ...
            if ePlotBonus == -1 and _getBonusCultivationChance(iPlayer, pLoopPlot, eBonus, False) > 0:
...
def isBonusCultivatable(pUnit):
    ...
    if pPlot.isCity():
        ...
        return _getBonusCultivationChance(pUnit.getOwner(), pPlot, eBonus) > 0
...
def _bonusIsCultivatableFromCity(iPlayer, pCity, eBonus, bVisibleOnly=True):
    for iI in range(gc.getNUM_CITY_PLOTS()):
        ...
        if pLoopPlot != None and not pLoopPlot.isNone():
            ePlotBonus = pLoopPlot.getBonusType(-1)
            if ePlotBonus == -1 and _getBonusCultivationChance(iPlayer, pLoopPlot, eBonus, bVisibleOnly) > 0:
                return True
    return False
                ...
                if pLoopPlot != None and not pLoopPlot.isNone():
                    ...
                    if ePlotBonus == -1 or bAlreadyImproved or iPass > 0:
                        ...
                        if eImprovement == -1 or bAlreadyImproved or iPass > 0:
                            if _getBonusCultivationChance(iPlayer, pLoopPlot, eBonus) > 0:
                                if iSkipN > 0:
                                    iSkipN -= 1
                                    continue
                                if iSkipN <= 0:
...
def doCultivation_AI(pUnit):
    ...
    while pLoopCity:
        ...
        if iDistance != -1:
            if _isCityCultivationPossible(pLoopCity):
                if iDistance == 0:
                    iValue = 2
                else:
                    iValue = 1/iDistance
        if iValue != 0:
            lCities.append((iValue, pLoopCity))
