    ...
    if not bInitialized:
        ...
        for i in range(iRange):
            ...
            if loopPlayer.isAlive():
                (pLoopCity, pIter) = loopPlayer.firstCity(False)
                while pLoopCity:
                    if int(CvUtil.getScriptData(pLoopCity, ["tsb"], -1)) != -1:
                        lCitiesSpecialBonus.append(pLoopCity)
                        if len(lCitiesSpecialBonus) == iMaxCitiesSpecialBonus:
                            break
                    (pLoopCity, pIter) = loopPlayer.nextCity(pIter, False)
            if len(lCitiesSpecialBonus) == iMaxCitiesSpecialBonus:
                break
...
def doSellBonus(pUnit, pCity):
    ...
    if eBonus != -1:
        ...
        if iOriginCiv != iBuyer:
            ...
            iGewinnWissen = int(iPrice / 4 * pSeller.getCurrentEra())
            doResearchPush(iBuyer, iGewinnWissen)
            doResearchPush(iSeller, iGewinnWissen)
...
def getBonusValue(eBonus):
    ...
    if eBonus in lCorn + lLivestock + lPlantation: return 20
...
def getPossibleTradeCitiesForCiv(iPlayer1, iPlayer2, bWater):
    ...
    pPlayer2 = gc.getPlayer(iPlayer2)
    lCityList = []
    (pLoopCity, pIter) = pPlayer2.firstCity(False)
    while pLoopCity:
        if pLoopCity.isRevealed(iTeam1, 0):
            if bWater:
                if pLoopCity.isCoastal(4):
                    lCityList.append(pLoopCity)
            else: lCityList.append(pLoopCity)
        (pLoopCity, pIter) = pPlayer2.nextCity(pIter, False)
    return lCityList
                        ...
                        # wenn Distanz null ist, wurde der letzte Plot gefunden
                            ...
                            if iTyp == 0:
                                if loopPlot.getRouteType() == iTradeRoad:
                                    return None
                                return loopPlot
                            elif loopPlot.getRouteType() == iTradeRoad:
                                return True
                            return False
                        # ---------
...
def getPlayerLuxuryCities(iPlayer):
    pPlayer = gc.getPlayer(iPlayer)
    lCityList = []
    (pLoopCity, pIter) = pPlayer.firstCity(False)
    while pLoopCity:
        lLuxuryGoods = getCityLuxuries(pLoopCity)
        if lLuxuryGoods:
            lCityList.append([pLoopCity, lLuxuryGoods])
        (pLoopCity, pIter) = pPlayer.nextCity(pIter, False)
    return lCityList
...
def addCityWithSpecialBonus(iGameTurn):
    ...
    if len(lCitiesSpecialBonus) >= iMaxCitiesSpecialBonus: return
    ...
    for i in range(iRange):
        ...
        if loopPlayer.isAlive() and not loopPlayer.isHuman():
            (pLoopCity, pIter) = loopPlayer.firstCity(False)
            while pLoopCity:
                if pLoopCity not in lCitiesSpecialBonus:
                    lNewCities.append(pLoopCity)
                (pLoopCity, pIter) = loopPlayer.nextCity(pIter, False)
    ...
    while lNewCities and iTry < 3:
        ...
        if lNewBonus:
            ...
            CyInterface().addMessage(gc.getGame().getActivePlayer(), True, 10, CyTranslator().getText("TXT_KEY_MESSAGE_TRADE_SPECIAL_2", (pCity.getName(), gc.getBonusInfo(eBonus).getDescription())), None, 2, None, ColorTypes(11), 0, 0, False, False)
            break
        else:
            iTry += 1
...
def doCheckCitySpecialBonus(pUnit, pCity, eBonus):
    ...
    if eCityBonus != -1 and eCityBonus == eBonus:
        ...
        # Choose units
        iNewUnit1 = lGift[CvUtil.myRandom(len(lGift))]
        iNewUnit2 = lGift[CvUtil.myRandom(len(lGift))]
        iNewUnit3 = lGift[CvUtil.myRandom(len(lGift))]

        # Create units
        pPlayer.initUnit(iNewUnit1, pCity.getX(), pCity.getY(), iNewUnitAIType, DirectionTypes.DIRECTION_SOUTH)
        pPlayer.initUnit(iNewUnit2, pCity.getX(), pCity.getY(), iNewUnitAIType, DirectionTypes.DIRECTION_SOUTH)
        pPlayer.initUnit(iNewUnit3, pCity.getX(), pCity.getY(), iNewUnitAIType, DirectionTypes.DIRECTION_SOUTH)
