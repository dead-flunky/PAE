...
def doVikings():
    ...
    for _ in range(4):
        ...
        if loopPlot != None and not loopPlot.isNone() and loopPlot2 != None and not loopPlot2.isNone():
        ...
        if gc.getGame().getGameTurnYear() >= 250 and gc.getGame().countCivPlayersAlive() < iMaxPlayers and False:
            ...
            if iPlayerBestTechScore > -1:
                ...
                for iTech in range(iTechNum):
                    if gc.getTechInfo(iTech) != None and xTeam.isHasTech(iTech) and not pTeam.isHasTech(iTech) and gc.getTechInfo(iTech).isTrade():
